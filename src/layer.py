...
class Layer:
    def __init__(self, packet: Packet, config: LayerConfig,
                 size_total: int, size_payload: int) -> None:
        self.packet = packet
    ...
    @property
    def down(self):
        return self.packet.layers[self.layer_type +
                                  1] if self.layer_type < len(self.packet.layers) - 1 else None
